...
def get_shift_time(timestamps, shift_type):
    ...
    if shift_type == 'day':
        shift_start = datetime.datetime(earliest_time.year, earliest_time.month, earliest_time.day, 6)
        shift_end = datetime.datetime(earliest_time.year, earliest_time.month, earliest_time.day, 18)
    else:
        shift_start = datetime.datetime(earliest_time.year, earliest_time.month, earliest_time.day, 18)
        shift_end = datetime.datetime(earliest_time.year, earliest_time.month, earliest_time.day, 6)
...
def get_velocities(df, time_subsampling_rate):
    ...
    same_reference_frame_mask = (ref_frame_cat[1:] == ref_frame_cat[:-1]) * (ref_frame_ind[1:] == ref_frame_ind[:-1])    
    ...
    velocities = velocities[correct_velocity_mask]
    data_mask[data_mask] = correct_velocity_mask
    
    return velocities, velocity_timestamps, data_mask
